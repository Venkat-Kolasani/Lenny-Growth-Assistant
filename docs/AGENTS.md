...
Read this file in full before touching this repo. It's the standing reference for any coding agent (or Venkat) picking up work in a new session — what's decided, what's not, and the protocol for keeping `handoff.md` and `decisions.md` honest as work progresses. Canonical copy lives under `docs/`; root `AGENTS.md` / `CLAUDE.md` only point here.
...
3. **On any non-obvious decision** (a trade-off, a deviation from the docs, a workaround): append a short rationale entry to `decisions.md` — this is Venkat's interview prep, so write it as "why," not just "what."
...
- **Agent-layer model routing:** the Skills/subagent/tool-use *architecture* follows the Claude Agent SDK's patterns; default inference is Groq/Ollama/Cloudflare. BYOK Anthropic uses the official `anthropic` Messages API (`backend/app/agent/anthropic_provider.py`). The coding-agent `claude-agent-sdk` harness is not the chat runtime — see `architecture.md` §4.
...
See the tree in the root `README.md`. Product and process markdown lives in `docs/` — keep new docs there rather than inventing new top-level folders without updating that tree.
