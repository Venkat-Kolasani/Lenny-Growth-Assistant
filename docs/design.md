...
- **Document card** — when an essay or brief is produced, chat shows a compact card (type, title, Export menu). Full markdown stays in the Artifact pane. Clicking the card focuses that artifact.
- **Export** — one **Export** control on the card and in the Artifact head opens Markdown (.md), Word (.docx), or PDF. PDF still uses the print stylesheet (cream/ink/accent + Iowan/Palatino/Georgia) via the browser print dialog. Markdown and Word download immediately from the artifact content in the browser (no new library).
