...
3. **Contextualize** — before embedding, generate a short (1-2 sentence) prefix describing where the chunk sits. `--contextualize` is one Groq sentence **per episode**, copied onto every chunk, then a re-embed — 10k per-chunk Ollama calls are ~27h and Groq's free daily cap is ~1k. Per-chunk remains the upgrade if eval still misses. Running it through Groq (else local Ollama) keeps it free.
...
**A note on how this stays free:** Anthropic's API has no ongoing free tier — new accounts get a one-time, 30-day trial credit, not a renewing allowance like Groq or Cloudflare. The full `claude-agent-sdk` package is a coding-agent harness (CLI + tools), not a chat-completion client, so it is the wrong runtime for turn-based RAG. The split is: skills/routing follow Agent SDK patterns (skill definitions, subagent boundaries, a structured tool-use loop), while default inference uses Groq and Ollama natively. **BYOK Anthropic** is a real Messages API adapter (`anthropic` Python SDK). It appears in the provider toggle only when `ANTHROPIC_API_KEY` is set. No key → Groq or local Ollama. Never required to run the app for free.
...
- **Anthropic — BYOK, not the free default.** Set `ANTHROPIC_API_KEY` (optional `ANTHROPIC_MODEL`, default `claude-sonnet-4-5`). The UI lists it as unavailable until the key is present; `POST /sessions/{id}/provider` returns 400 without it. Never required to run the app.
