...
- **Backend:** FastAPI
- **Agent layer:** Skills-based routing that *follows* Claude Agent SDK patterns — not Pi Coding Agent, and not the SDK package at runtime. Rationale in `architecture.md` §4. Do not claim an Anthropic adapter is wired; it is not.
...
- **Agent-layer model routing:** the Skills/subagent/tool-use *architecture* follows the Claude Agent SDK's patterns; actual inference calls Groq/Ollama/Cloudflare natively (`ANTHROPIC_BASE_URL` tricks sit in an unresolved ToS gray area — see `architecture.md` §4). `ANTHROPIC_API_KEY` is reserved in settings only. There is no Agent SDK package and no Anthropic complete() adapter in this repo.
